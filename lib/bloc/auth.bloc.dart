...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  AuthBloc() : super(AuthInitial()) {
    on<AuthSendOtp>((event, emit) async {
      emit(AuthLoading());
      try {
        final Response response = await network.sendOtp(event.phonNumber);
    ...
    on<VerifyOtpEvent>((event, emit) async {
      ...
      try {
        Response response = await network.verfiyOtp(
            phonenumber: event.phonNumber!, code: event.code!);
        var authResponse = UserModel.fromJson(response.data);
        if (response.statusCode == 201) {
